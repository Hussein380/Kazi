          ...
          <div className="hidden md:flex md:items-center md:gap-6 md:justify-center md:flex-1">
            {userRole === "employer" && (
              <>
                ...
                <NavLink to="/employees">Browse Employees</NavLink>
          ...
          <div className="md:hidden border-t p-4 space-y-4 bg-background animate-in slide-in-from-top-4">
            {userRole === "employer" && (
              <div className="flex flex-col gap-3">
                ...
                <Link to="/employer/jobs" onClick={() => setIsMenuOpen(false)}>
                  My Jobs
                </Link>
                <Link to="/employees" onClick={() => setIsMenuOpen(false)}>
                  Browse Employees
                </Link>
                <Link to="/jobs/new" onClick={() => setIsMenuOpen(false)}>
                  Post Job
                </Link>
                <Link to="/attestations" onClick={() => setIsMenuOpen(false)}>
                  Attestations
                </Link>
              </div>
            )}
            ...
            {userRole === "worker" && (
              <div className="flex flex-col gap-3">
                ...
                <Link to="/workers" onClick={() => setIsMenuOpen(false)}>
                  Browse Workers
                </Link>
                <Link to="/cv/generate" onClick={() => setIsMenuOpen(false)}>
                  CV Generator
                </Link>
              </div>
            )}
